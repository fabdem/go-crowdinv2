...
// AddDirectory - Add directory API call. Add a directory to a project.
// {protocol}://{host}/api/v2/projects/{projectId}/directories
func (crowdin *Crowdin) AddDirectory(options *AddDirectoryOptions) (*ResponseAddDirectory, error) {

	crowdin.log("\nAddDirectory()")

	// Prepare URL and params
	var p postOptions
	p.urlStr = fmt.Sprintf(crowdin.config.apiBaseURL + "projects/%v/directories", crowdin.config.projectId)
	p.body = options
	...
	var responseAPI ResponseAddDirectory
	err = json.Unmarshal(response, &responseAPI)
	if err != nil {
		crowdin.log(err)
		return nil, err
...
// EditFile - Edit File details
// {protocol}://{host}/api/v2/projects/{projectId}/files/{fileId}
//

func (crowdin *Crowdin) EditFile(options *EditFileOptions, stringId int) (*ResponseEditFile, error) {
